        ...
        <x-admin.components.card heading="Customer Information">
            <form method="POST" wire:submit.prevent="updateCustomer" class="flex justify-between flex-col">
                <div class="grid grid-cols-1 gap-4">
                    <x-admin.components.input.group label="{{ __('inputs.firstname') }}" for="firstname"
                                                    :error="$errors->first('customer.first_name')">
                        <x-admin.components.input.text wire:model.defer="customer.first_name" name="firstname"
                                                       id="firstname" :error="$errors->first('customer.first_name')"/>
                    </x-admin.components.input.group>
                    <x-admin.components.input.group label="{{ __('inputs.lastname') }}" for="lastname"
                                                    :error="$errors->first('customer.last_name')">
                        <x-admin.components.input.text wire:model.defer="customer.last_name" name="lastname"
                                                       id="lastname" :error="$errors->first('customer.last_name')"/>
                    </x-admin.components.input.group>
                    <x-admin.components.input.group label="{{ __('inputs.username') }}" for="name"
                                                    :error="$errors->first('customer.username')">
                        <x-admin.components.input.text wire:model.defer="customer.username" name="name" id="name"
                                                       :error="$errors->first('customer.username')"/>
                    </x-admin.components.input.group>
                    <x-admin.components.input.group label="{{ __('inputs.email') }}" for="email">
                        <x-admin.components.input.text value="{{ $this->customer->email }}" type="email" name="email"
                                                       readonly id="email"/>
                    </x-admin.components.input.group>
                </div>
                            ...
                            <x-admin.components.table.cell>
                                @if ($vendorRequest->status->name == 'Pending')
                                    <x-admin.components.button wire:loading type="button"
                                                               wire:key="vendor_request_{{ $vendorRequest->id }}"
                                                               @click="$dispatch('showslideover', {{ $vendorRequest->id }})">
                ...
                <x-slot name="body">
                    @forelse ($badgeRequests as $badgeRequest)
                        <x-admin.components.table.row wire:loading.class.delay="opacity-50">
                            ...
                            <x-admin.components.table.cell>
                                <a href="{{route('admin.badges.show',$badgeRequest->badge_id)}}">{{ $badgeRequest->badge->name }}</a>
                            </x-admin.components.table.cell>
                            <x-admin.components.table.cell>
                                {{$badgeRequest->requested_at->format('m/d/Y H:m:s')}}
                            </x-admin.components.table.cell>
                            <x-admin.components.table.cell>
                                <span
                                    class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100">
                        ...
                        <x-admin.components.table.row>
                            <x-admin.components.table.cell colspan="4">
                                <div class="flex justify-center items-center space-x-2">
                                    <span
                                        class="font-medium py-8 text-gray-400 text-xl">{{ __('global.no_data') }}</span>
                                </div>
                            </x-admin.components.table.cell>
                        </x-admin.components.table.row>
                    @endforelse
